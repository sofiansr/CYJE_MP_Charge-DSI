...
> [!NOTE]
> Chaque modification de la base de données des prospects ou des utilisateurs est susceptible de modifier le contenu d'une ou plusieurs de ces cases.
...
Un **prospect** (une ligne) peut être composé de **plusieurs contacts** (voir colonnes ```Nom(s)```, ```Prénom(s)```, ```Email(s)```, ```Tel(s)```, ```Poste(s)```).</br>
Ce tableau est composée de 17 colonnes :
...
| Statuts | Statut actuel du prospect.</br>Les statuts possibles sont :</br> ```A contacter```, ```Contacté```, ```A rappeler```, ```Relancé```, ```RDV```, ```PC```, ```Signé```, ```PC refusée```, ```Perdu``` |
...
Vous pouvez également modifier ou supprimer un utilisateur, en le choississant dans la liste, puis en cliquant sur le bouton ```Modifier```.</br>
Le bouton ```Modifier``` débloquera les champs, vous permettant de les modifier. N'oubliez pas **d'enregistrer ou d'annuler** vos modifications.</br>
